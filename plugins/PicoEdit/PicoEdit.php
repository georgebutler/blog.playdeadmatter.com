...
final class PicoEdit extends AbstractPicoPlugin {
  ...
  {
    $twig_vars['PicoEdit_url'] = $this->getPageUrl( $this->url );
    if( $this->is_logout ) {
      session_destroy();
      header( 'Location: '. $twig_vars['PicoEdit_url'] );
      exit;
    }
  ...
  public function onConfigLoaded( array &$config ) {
    // Default options
    if( !isset( $config['PicoEdit_404'] ) ) $config['PicoEdit_404'] = TRUE;
    if( !isset( $config['PicoEdit_options'] ) ) $config['PicoEdit_options'] = TRUE;
    // Parse extra options
  ...
  public function onRequestUrl( &$url ) {
    // If the request is anything to do with PicoEdit, then we start the PHP session
  ...
  private function do_new()
  {
    ...
    $error = '';
    // if( is_null( $content ) ) {
    $content = "/*\nTitle: $name\nAuthor: " . ( $this->getConfig( 'PicoEdit_default_author' ) ? $this->getConfig( 'PicoEdit_default_author' ) : '' ) . "\nDate: ". date('j F Y') . "\n*/\n\n";
    // }
  ...
  private function do_open() {
    ...
    else if( $this->getConfig( 'PicoEdit_options' ) )
    {
      $conf = $this->getConfigDir() . '/options.conf';
  ...
  {
    if(file_exists('./plugins/PicoEdit/commitform.html')) {
      # Do the git stuff...
      ...
      $loader = new Twig_Loader_Filesystem('./plugins/PicoEdit');
      $twig = new Twig_Environment($loader, array('cache' => null));
      $twig->addExtension(new Twig_Extension_Debug());
      $twig_vars = array(
        'status' => $status,
  ...
  private function do_commit_post()
  {
    ...
    if(file_exists('./plugins/PicoEdit/commitresponse.html')) {
      $loader = new Twig_Loader_Filesystem('./plugins/PicoEdit');
      $twig = new Twig_Environment($loader, array('cache' => null));
      $twig->addExtension(new Twig_Extension_Debug());
      $twig_vars = array(
  ...
  {
    if(file_exists('./plugins/PicoEdit/pushpullform.html')) {
      # Do the git stuff...
      require_once 'Git-php-lib';
      $repo = Git::open('.');
      $remotes = '';
  ...
  {
    if(file_exists('./plugins/PicoEdit/pushpullresponse.html')) {
      # Do the git stuff...
      require_once 'Git-php-lib';
      $repo = Git::open('.');
      $remotes = array();
